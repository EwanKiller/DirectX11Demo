...
namespace 
{
	// 用以从Windows全局窗口转发消息到我们自己的成员函数窗口
	...
	, m_pDepthStencilView(nullptr)
{
	...
	/* 让一个全局指针获取这个类，可以在Windows消息处理的回调函数
	 让该类调用内部的回调函数*/
	g_pD3DApp = this;
}
...
void D3DApp::OnResize()
{
	...
	/* 重新设置交换链，重新创建渲染目标视图 */
	ComPtr<ID3D11Texture2D> backBuffter;
	HR(m_pSwapChain->ResizeBuffers(1, m_ClientWidth, m_ClientHeight, DXGI_FORMAT_R8G8B8A8_UNORM, 0));
	HR(m_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(backBuffter.GetAddressOf())));
	HR(m_pD3DDevice->CreateRenderTargetView(backBuffter.Get(), nullptr, m_pRenderTargetView.GetAddressOf()));

	// 设置调试对象名
	D3D11SetDebugObjectName(backBuffter.Get(), "BackBuffer[0]");
...
LRESULT D3DApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		/*
		WM_ACTIVE is sent when the window is active or deactivated.
		We pause the game when the window is deactivated(inactive)
		We unpause it when the window becomes active
		*/
	case WM_ACTIVATE:
	...
	case WM_SIZE:
		...
		if (m_pD3DDevice)
		{
			...
			else if (wParam == SIZE_RESTORED)
			{
				...
				// API call such as SetWindowPos or m_pSwapChain->SetFullScreenState
				else
				{
					OnResize();
				}
	...
	case WM_EXITSIZEMOVE:
		...
		OnResize();
		return 0;
		// WM_DESTORY is sent when the window is being destroyed
	case WM_DESTROY:
		PostQuitMessage(0);
		return 0;
		// The WM_MENUCHAR message is sent when a menu is active and the user presess
		// a key that does not correspond to any mnemonic or accelerator key.
	case WM_MENUCHAR:
		// dont beep when we alt-enter
		return MAKELRESULT(0, MNC_CLOSE);
		// Catch this message so to prevent the window from becoming to small. 
...
bool D3DApp::InitDirect3D()
{
	...
	for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++)
	{
		...
		if (hr == E_INVALIDARG)
		{
			// D3D 11.0的API不承认D3D_FEATURE_LEVEL_11_1,需要尝试特性等级11.0及以下的版本
	...
	ComPtr<IDXGIFactory1> dxgiFactory1 = nullptr; // D3D11.0(包含DXGI11.1)的接口类
	ComPtr<IDXGIFactory2> dxgiFactory2 = nullptr; // D3D11.1(包含DXGI11.2)特有的接口类

	// 为了正确创建 DXGUI交换链，首先我们需要获取创建 D3D设备 的DXGI工厂，否则会报错：
	// "IDXGIFactory::CreateSwapChain:This function is being called with a device from a different IDXGIFactor."
